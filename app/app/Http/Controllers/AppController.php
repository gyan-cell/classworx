...
//        }
 ...
 public function codeEditor()
    {
        return view('app.code-editor',[
            'navigation' => 'code-editor',
            'page_title' => __('Code Editor'),
            'page_subtitle' => __('Code Anything'),
]);
    }
    
    
     public function algoviz()
    {
        return view('app.algo-visual',[
            'navigation' => 'algo-visual',
            'page_title' => __('Algo Viz'),
            'page_subtitle' => __('Visualize the Algorithm'),
]);
    }
...
public function stopTimer()
{
    ...
    if ($this->timer && $this->timer->is_running) {
        $timer = $this->timer;
        $timer->is_running = 0;
        $timer->timer_stopped_at = Carbon::now();
        $timer->timer_duration = $timer->timer_started_at->diffInSeconds($timer->timer_stopped_at);
    ...
    return response()->json(['status' => 'Timer stopped due to inactivity']);
}
